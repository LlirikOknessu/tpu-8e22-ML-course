...
def preprocess_data(X):
    ...
    # One-hot decoding
    X['type_encoded'] = X[one_hot_columns].idxmax(axis=1).str.split('_').str[1]
    label_mapping = {label: idx for idx, label in enumerate(sorted(X['type_encoded'].unique()))}
    X['type_encoded'] = X['type_encoded'].map(label_mapping)
